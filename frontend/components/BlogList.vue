    ...
    <div v-if="blogs" class="mb-10 lg:mb-20">
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5 mt-5">
            <div v-for="item in blogs" :key="item.id" class="rounded-lg overflow-hidden">
                <NuxtImg 
                    :src="item.image"
                    class="w-full h-60 object-cover transition-transform duration-300 ease-in-out transform hover:scale-105 hover:brightness-110"
                    alt="image blog" quality="80" loading="lazy" />
                <div class="bg-blog_light_bg/70 p-5">
                    <div class="text-xs flex justify-between items-center gap-5">
                        ...
                        <div>
                            <Badge>{{ item?.blog_type.toLocaleUpperCase() }}</Badge>
                        </div>
                    </div>
                    <NuxtLink 
                       :to="{ name: 'blogs-slug', params: { slug: item?.slug } }"
                        ...
                        {{ item?.title }}
                        <p class="line-clamp-2 font-normal hover:text-gray-700 h-11">
                            {{ item?.description }}
                        </p>
                    </NuxtLink>
                </div>
